...
A Single-Dataset Stress-Test Framework for Flow-Based Network Intrusion Detection}
...
This manuscript evaluates whether a single CICIDS2017-like dataset can still support a strong scientific contribution if the paper is framed as a benchmark-validity and deployment-reliability study rather than as a new detector paper. We use only the locally available CSV files, but we compensate with stress tests designed to approximate deployment shifts: temporal holdout, latest-day holdout, Web scenario holdout, endpoint-pair holdout and unknown-family open-set holdout. We also record calibration, abstention and explanation-stability behavior.
...
\paragraph{Single-dataset limitation.}
The most important limitation is the use of one CICIDS2017-like dataset. The study cannot claim universal cross-dataset generalization. Its contribution is a reproducible intra-dataset stress-test framework.
...
The current open-set analysis uses classifier uncertainty. It should be considered a diagnostic baseline, not a state-of-the-art open-set method.
...
FAIR-ML-CYBER shows that a CICIDS2017-like dataset can still support a meaningful Q1-oriented contribution if the paper is framed as a rigorous evaluation and benchmark-validity study. The verified results show that random-split scores are overly optimistic, temporal and scenario shifts expose major transferability failures, calibration degrades under shift, rare classes are poorly detected, open-set behavior is inconsistent and explanation stability depends on the split protocol.
