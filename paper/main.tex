...
\begin{enumerate}[leftmargin=*]
  ...
  \item A seed-42 advanced analysis covering multiclass labels, rare classes, unknown attack families, calibration, abstention and explanation stability.
  \item A partial external validation on CSE-CIC-IDS2018, including binary stress tests, post-hoc calibration and anomaly-based open-set baselines.
...
We also downloaded five public CSE-CIC-IDS2018 CICFlowMeter CSV files from the official UNB/AWS source \cite{unbCSECICIDS2018}. The audited subset contains 4,525,400 rows after removing repeated embedded header rows and has hash \texttt{414f7195ee1e137a}. The downloaded subset contains Benign, Bot, FTP-BruteForce, SSH-Bruteforce, Infilteration, Brute Force-Web, Brute Force-XSS and SQL Injection labels. Because benign traffic dominates, we generated a real-data stratified validation sample with 80,000 benign rows, up to 30,000 rows per attack label and all rare Web/XSS/SQL rows. The resulting sample contains 200,928 rows and has hash \texttt{d90aca82728746b3}. This sample is not synthetic. It is a capped subset of real CSE-CIC-IDS2018 rows. Endpoint-pair holdout is unavailable for this subset because the public processed CSV files do not include source and destination IP columns.
...
Second, binary evaluation is insufficient. Multiclass macro-F1, rare-class results and Web holdout results reveal failures that are hidden by weighted or binary metrics. This is particularly important for SOC deployment, where missing a rare but critical attack can be operationally unacceptable.
...
\paragraph{External-validation scope.}
The main evidence still comes from one CICIDS2017-like dataset. The CSE-CIC-IDS2018 result is a partial validation using five public CSV files and a stratified real-data sample, not a full cross-dataset benchmark. The paper should therefore claim external consistency evidence, not universal generalization.

\paragraph{Known CICIDS2017 limitations.}
Prior studies report CICIDS2017 flow-construction and labelling issues. These issues motivate the study but also limit claims about real-world attack detection.

\paragraph{Advanced analyses are seed 42 only.}
The binary core is validated on seeds 42, 7 and 99. The advanced analyses are currently seed 42 only. Final claims about rare-class, open-set, calibration and explanation stability should be repeated on additional seeds if compute budget allows.

\paragraph{LogisticRegression convergence.}
LR convergence is fixed for seed 42 with \texttt{max\_iter=2000}. Older seed 7 and seed 99 LR rows still contain warnings from \texttt{max\_iter=500}. Corrected LR2000 reruns without the timestamp-derived \texttt{hour} feature have been submitted for seeds 42, 7 and 99, but their artifacts were not yet available at the time of this draft. Final LR multi-seed tables should use those convergence-clean reruns or explicitly report this limitation.

\paragraph{Open-set baseline strength.}
The main advanced open-set analysis uses classifier uncertainty. The external validation adds Isolation Forest and Local Outlier Factor, but these are still baseline anomaly detectors. A final version should compare additional open-set or anomaly approaches if scope permits.
