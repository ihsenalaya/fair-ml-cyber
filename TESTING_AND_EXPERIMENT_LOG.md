...
| 2026-06-17 | Prepared `azureml/full_core_memory_job.yml` | Not submitted yet in this log entry | Full data planned | `logistic_regression`, `hist_gradient_boosting` | 5 validated splits | 2 tiers | Same full-core protocol, but compute is `azureml:cpu-memory-cluster` (`Standard_E8ds_v5`) and result prefix is `fullcore_mem_s42`. |
...
| 2026-06-17 | Azure ML full-data job | `fullcore-s42-001` failed before completing first model | Azure ML reported Python killed by `SIGKILL`, possibly out of memory, on `Standard_DS3_v2` after starting first full-data logistic regression run | Downloaded partial logs; created memory-optimized compute `cpu-memory-cluster` with `Standard_E8ds_v5`; prepared `azureml/full_core_memory_job.yml` | Open until memory rerun completes | `Standard_DS3_v2` must not be claimed sufficient for full-data final experiments |
